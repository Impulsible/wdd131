...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Toggle menu open/close + update icon and aria
  function toggleMenu() {
    const expanded = hamburger.getAttribute("aria-expanded") === "true";
    ...
    hamburger.textContent = newExpanded ? "✖" : "☰";
  }

  hamburger.addEventListener("click", toggleMenu);

  // Close menu on nav button click + filter temples
  navMenu.querySelectorAll("button").forEach((button) => {
    button.addEventListener("click", () => {
      if (window.innerWidth <= 600) {
        // Close menu
      ...
      // Call your existing filtering logic (assuming filterTemples and renderTemples exist)
      const filterId = button.id;
      const filteredTemples = filterTemples(filterId);
      renderTemples(filteredTemples);
    });
  ...
  // Hide menu when scrolling down, keep it available on scroll up (mobile only)
  window.addEventListener("scroll", () => {
    ...
    if (currentScrollY > lastScrollY && navMenu.classList.contains("active")) {
      // Scrolling down, hide menu if open
      toggleMenu();
    }
